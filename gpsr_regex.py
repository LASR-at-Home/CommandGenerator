...
def gpsr_components():
    ...
    pose_person_plural_list = ["sitting persons",
                               "standing persons", "lying persons"]

    person_info_list = ["name", "pose", "gesture"]
    object_comp_list = ["biggest", "largest",
    ...
    clothes_list = ["t shirts", "shirts",
                    "blouses", "sweaters", "coats", "jackets"]
    color_clothe_list = []
    ...
    gesture_person_plural_list = list_to_regex(
        gesture_person_plural_list, 'gesture')
    pose_person_plural_list = list_to_regex(
        pose_person_plural_list, 'pose')
    ...
    return verb, prep, "(?:an|a)", connector_list, gesture_person_list, pose_person_list, gesture_person_plural_list, pose_person_plural_list, person_info_list, object_comp_list, talk_list, question_list, color_clothe_list, color_clothes_list


def gpsr_regex(configuration: Configuration):
    verb, prep, art, connector_list, gesture_person_list, pose_person_list, gesture_person_plural_list, pose_person_plural_list, person_info_list, object_comp_list, talk_list, question_list, color_clothe_list, color_clothes_list = gpsr_components()
    ...
    command(
        'goToLoc', f"{verb('go')} {prep('toLocPrep')} the {Configuration.pick(configuration, 'location', ['loc', 'room'])} then {SUB_COMMAND_TODO_UNIMPLEMENTED()}")
    command('takeObjFromPlcmt',
    ...
    command('followNameFromBeacToRoom',
    ...
    command('guideNameFromBeacToBeac',
    ...
    command('guidePrsFromBeacToBeac',
    ...
    command('guideClothPrsFromBeacToBeac',
    ...
    command('meetNameAtLocThenFindInRm',
...
if __name__ == "__main__":
    ...
    print(execute('go to the sofa then '))
    print(execute('take a cup from the kitchen table and '))
    print(execute('find a waving person in the living room and '))
    print(execute('find a television in the living room and '))
    print(execute('meet guest1 in the kitchen and '))
    print(execute('tell me how many sticks there are on the kitchen table'))
    print(execute('tell me how many waving persons are in the living room'))
    print(execute('tell me the name of the person in the kitchen'))
    print(execute('tell me what is the biggest object on the kitchen table'))
    print(execute('say something about yourself to the person pointing to the left in the living room'))
    print(execute('answer the quiz of the waving person in the kitchen'))
    print(execute('follow guest1 from the sofa to the kitchen'))
    print(execute('guide guest2 from the sofa to the living room'))
    print(execute('take the sitting person from the sofa to the piano'))
    print(execute('escort the person wearing a red shirt from the sofa to the kitchen'))
    print(execute('bring me a cup from the kitchen table'))
    print(execute('tell me what is the heaviest stick on the kitchen table'))
    print(execute('say hello to the person wearing a black shirt in the kitchen and '))
    print(execute('introduce yourself to guest2 in the living room and '))
    print(execute(
        'meet guest2 at the living room then locate them in the kitchen'))
    print(execute('tell me how many people in the kitchen are wearing blue coats'))
    print(execute('follow the waving person at the sofa'))
